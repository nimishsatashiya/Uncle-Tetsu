	...
	public function startImport()
    {
        ...
        if($filterClassesCount > 0)
        {
            foreach ($filterClasses as $ArrClass)
            {
                ...
                $query = "(ModificationTimestamp=$lastDate+)";
                $query = "(ModificationTimestamp=$lastDate+),(MlsStatus=ACT)";
                    ...
                    if(!empty($rows)){
                    	foreach($rows as $value)
                        {
                            if($value['MlsStatus'] == 'Active')
                            {
                                ...
                                {
                                    $RETS_path = base_path().'/public/RETS/';
                                    $listing = $value['ListingKeyNumeric'];
